...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: order, error: orderError } = await userClient
      .from('orders')
      .select('id, customer_id, restaurant_name, items, total, payment_method, payment_status')
      .eq('id', orderId)
      .single();
    ...
    const origin = request.headers.get('origin') ?? Deno.env.get('APP_URL')!;
    const preferenceResponse = await fetch('https://api.mercadopago.com/checkout/preferences', {
      ...
      body: JSON.stringify({
        ...
        back_urls: {
          success: `${origin}/menu.html?payment=success`,
          pending: `${origin}/menu.html?payment=pending`,
          failure: `${origin}/menu.html?payment=failure`
        },
